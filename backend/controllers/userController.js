...
const bookAppointment = async (req, res) => {
  try {
    ...
    if (slots_booked[slotDate]) {
      const bookedSlots = slots_booked[slotDate].map(normalizeTime);
      if (bookedSlots.includes(normalizedSlotTime)) {
        return res.json({ success: false, message: 'Slot not available' });
      } else {
        slots_booked[slotDate].push(normalizedSlotTime);
      }
    } else {
      slots_booked[slotDate] = [normalizedSlotTime];
    }
    ...
    const transporter = nodemailer.createTransport({
      ...
      auth: {
        ...
        pass: 'mvhv trpw thcj bepz', // Use App Password, not your actual Gmail password
      },
    });
